<template>
  ...
  <v-dialog v-model="createDialog" max-width="500px">
    <v-card>
      <v-card-title>
        <span class="headline">Crear Solicitante</span>
      </v-card-title>
      <v-card-text>
...
import { useSolicitantesStore } from '@/stores/solicitantesSlide';
...
const saveEdit = async () => {
  if (editedItem.value.nombre) {
    ...
    const response = await axios.put(`/solicitantes/${editedItem.value.solicitanteId}`, values);
    if (response) {
...
const confirmDelete = async () => {
  const response = await axios.delete(`/solicitantes/${deleteItem.value.solicitanteId}`);
  if (response) {
...
const saveCreate = async () => {
  if (nameNewItem.value) {
    const values = {
      Nombre: nameNewItem.value,
      UsuarioRegistro: getUserId()
    }

    const response = await axios.post(`/solicitantes`, values);
    if (response) {
...
const goTo = (item) => {
  router.push(`/documentos/${item.solicitanteId}`)
}
